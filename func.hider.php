...
if (file_exists($f = $_SESSION["f"]["path"]))
{
    ...
    printf("File: %s<br />", basename($_SESSION["f"]["name"]));
...
<form method="post">
    <div style="margin: 20px">
        <label><input type="radio" name="type" value="funcs" checked>Скрыть только функции</label><br />
        <label><input type="radio" name="type" value="code">Скрыть полностью весь код</label><br />
        <label><input type="radio" name="type" value="dismiss">Отменить действие и удалить файл</label><br />
    </div>
    <button type="submit" style="padding: 10px; padding-left: 140px; padding-right: 140px;">За дело</button>
</form>
